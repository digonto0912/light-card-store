...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="single-product card">
      ...
      <p>Total Rating: <span class="gold">${product.rating.rate}</span></p>
      <p>Rateing Counts: <span class="gold">${product.rating.count}</span></p>
      <h2>Price: $ ${product.price}</h2>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="s-btn">add to cart</button>
      <button id="details-btn" class="w-btn">Details</button></div>
      </div>
      `;
